...
def custom_soup_and_loop_logic(spider, response_body, url, BeautifulSoup):
    ...
    span = 'span[class]:not(.visually-hidden,.link__type,.hds-button__label)'
    css_selector = (
        '.user-edited-content:not(.accordion) '+span+','
        '.user-edited-content.accordion .helfi-accordion__content__inner '+span+''
    )
    ...
    for match in matches:
        ...
        design_attr = match.get('data-design')
        yield {
            'class': class_attr if class_attr is not None else '',
            'design': design_attr if design_attr is not None else '',
            'url': url,
