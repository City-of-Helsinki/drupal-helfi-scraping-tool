...
regex_content_include_pattern = r'contact-card--with-image'
...
def custom_soup_and_loop_logic(spider, response_body, url, BeautifulSoup):
    ...
    css_selector = '.contact-card--with-image img'

    # Find and loop through matching elements on this page
    matches = soup.select(css_selector)
    for match in matches:
        test = match['src']
        src_match = re.search(r'styles/([^/]*)/', test)
        if src_match:
            src_style = src_match.group(1)
        else:
            src_style = None
